...
class PokemonPauseMenu_Scene
  ...
  def pbStartScene
    ...
    @sprites["background"].blur
    @sprites["background"].bitmap.blt(0, 0, SpriteHash.bitmap("Graphics/Pictures/PauseMenu/bg"),Rect.new(0, 0, Graphics.width, Graphics.height))
    bmp = SpriteHash.bitmap("Graphics/Pictures/Common/scrollbar_bg")
    ...
    @sprites["panorama"] = Sprites::Scrolling.new(@viewport)
    @sprites["panorama"].set_bitmap("Graphics/Pictures/Common/panorama")
    ...
    @sprites["textOverlay"] = Sprites::Base.new(@viewport)
    ...
    bmp = SpriteHash.bitmap("Graphics/Pictures/Common/partyBar")
    content = []
    text = []
    if pbInSafari?
    ...
    @sprites["scroll"] = Sprites::Base.new(@viewport)
  ...
  def refresh
    ...
    bmp = SpriteHash.bitmap("Graphics/Pictures/PauseMenu/sel")
    for i in 0...@entries.length
      key = @entries[i]
      @sprites["#{i}"] = Sprites::Base.new(@viewport)
      ...
      for j in 0...2
        opac = j == 0 ? 155 : 255
        icon = SpriteHash.bitmap(ModularMenu.icon(key))
    ...
    bmp = SpriteHash.bitmap("Graphics/Pictures/Common/scrollbar_kn")
